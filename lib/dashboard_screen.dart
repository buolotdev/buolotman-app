...
class _DashboardScreenState extends State<DashboardScreen> {
  ...
  Future<void> _checkAndShowTour(
    ...
    if (_toursShownThisSession.contains(userId))
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (mounted) {
        ShowCaseWidget.of(
          showcaseContext,
        ).startShowCase([_tourKey1, _tourKey2, _tourKey3]);
      }
    });
  }
                              ...
                              _buildSectionHeader(
                                ...
                                onTap: () {
                                  Navigator.of(context).push(
                                    MaterialPageRoute(
                                      builder: (context) =>
                                          const BrowseProfessionalsScreen(),
                                    ),
                                  );
                                },
                              ),
  ...
  Widget _buildProfileCompletionBanner(AppState appState) {
    ...
    if (appState.verificationStatus == 'Verified')
            ...
            children: [
              ...
              GestureDetector(
                onTap: () {
                  Navigator.of(context).push(
                    MaterialPageRoute(
                      builder: (context) => const ServerNotificationsScreen(),
                    ),
                  );
                },
                child: Stack(
          ...
          return GestureDetector(
            onTap: () {
              Navigator.of(context).push(
                MaterialPageRoute(
                  builder: (context) =>
                      ListingScreen(initialQuery: service['title'] as String),
                ),
              );
            },
            child: Container(
        ...
        children: tasks.map((task) {
          ...
          return GestureDetector(
            onTap: () {
              Navigator.of(context).push(
                MaterialPageRoute(
                  builder: (context) => BrowseTasksScreen(taskId: id),
                ),
              );
            },
            child: Container(
      ...
      return Container(
        ...
        child: Column(
          children: [
            ...
            const SizedBox(height: 4),
            const Text(
              'Post a task to start finding professionals',
      ...
      return Container(
        ...
        child: Column(
          children: [
            ...
            const SizedBox(height: 4),
            const Text(
              'Hire professionals to build your team',
              style: TextStyle(color: Color(0xFF94A3B8), fontSize: 12),
              textAlign: TextAlign.center,
            ),
          ],
  ...
  Widget _buildCompanyServiceCatalog(AppState appState) {
      ...
      return Container(
        ...
        child: Column(
          children: [
            ...
            const SizedBox(height: 4),
            const Text(
              'Tap "Edit" above to post your first service',
              style: TextStyle(color: Color(0xFF94A3B8), fontSize: 12),
              textAlign: TextAlign.center,
            ),
          ],
    ...
    IconData _categoryIcon(String cat) {
      ...
      if (c.contains('it') || c.contains('tech') || c.contains('dev'))
  ...
  Widget _buildTopProfessionals(AppState appState) {
    ...
    final professionals = appState.publicPros.map((user) {
      ...
      final int completedJobs =
          int.tryParse(user['completed_jobs']?.toString() ?? '0') ?? 0;
      final double rate =
          double.tryParse(user['hourly_rate']?.toString() ?? '0') ?? 0.0;
      return {
        'name': name,
  ...
  Widget _buildFeaturedCompanies(AppState appState) {
    ...
    final companies = appState.publicCompanies.map((c) {
      ...
      final String industry =
          c['services_offered'] is List &&
              (c['services_offered'] as List).isNotEmpty
          ? (c['services_offered'] as List).first.toString()
          : 'Service Company';
      final double rating =
          double.tryParse(c['average_rating']?.toString() ?? '4.8') ?? 4.8;
          ...
          return GestureDetector(
            onTap: () {
              ...
              Navigator.of(context).push(
                MaterialPageRoute(
                  builder: (context) =>
                      CompanyProfileScreen(companyData: rawCompany),
                ),
              );
            },
            child: Container(
          ...
          children: [
            ...
            Row(
              children: [
                _buildMiniSummary(
                  'Services',
                  appState.services.length.toString(),
                ),
                const SizedBox(width: 10),
                _buildMiniSummary(
                  'Open Work',
                  appState.openMarketplaceTasks.length.toString(),
                ),
                const SizedBox(width: 10),
                        ...
                        children: [
                          CircleAvatar(
                            ...
                            backgroundImage:
                                ...
                                : null,
                            child:
                                (avatar.isEmpty || avatar.contains('onboard'))
                                ? const Icon(
                                    Icons.person,
                                    size: 28,
                                    color: Color(0xFF94A3B8),
                ...
                child: ListView(
                  ...
                  children: [
                    ...
                    if (role == 'Client') ...[
                      _buildDrawerItem(
                        ...
                        onTap: () {
                          ...
                          Navigator.of(context).push(
                            MaterialPageRoute(
                              builder: (_) => const PostTaskFormScreen(),
                            ),
                          );
                        },
                      ),
                    ...
                    if (role == 'Technician') ...[
                      _buildDrawerItem(
                        ...
                        onTap: () {
                          ...
                          Navigator.of(context).push(
                            MaterialPageRoute(
                              builder: (_) => const TaskFeedScreen(),
                            ),
                          );
                        },
                      ),
                    ...
                    if (role != 'Client') ...[
                      ...
                      ...appState.apiCategories.map(
                        (cat) => _buildDrawerItem(
                          context: context,
