...
class _PostServiceScreenState extends State<PostServiceScreen> {
    ...
    return Container(
      ...
      child: DropdownButtonHideUnderline(
        child: DropdownButton<String>(
          value: names.contains(_selectedCategory)
              ? _selectedCategory
              : names.first,
          isExpanded: true,
    ...
    final models = [
      {
        ...
        'desc': 'One set price for the whole service',
      },
      {
        'key': 'Hourly',
        'label': 'Hourly Rate',
        'icon': Icons.timer_outlined,
        'desc': 'Charge per hour of work',
      },
      {
        'key': 'Project',
        'label': 'Project-Based',
        'icon': Icons.assignment_outlined,
        'desc': 'Quote per project scope',
      },
    ];
    return Column(
    ...
    final types = [
      {
        ...
        'desc': 'You visit the client',
      },
      {
        'key': 'remote',
        'label': 'Remote',
        'icon': Icons.wifi_outlined,
        'desc': 'Service provided remotely',
      },
      {
        'key': 'both',
        'label': 'Both',
        'icon': Icons.swap_horiz,
        'desc': 'On-site and remote available',
      },
    ];
    return Row(
  ...
  void _handleNext() async {
    ...
    if (_currentStep == 2) {
      if (_priceMinController.text.trim().isEmpty &&
          _priceLabelController.text.trim().isEmpty) {
