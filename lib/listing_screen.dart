...
class _ListingScreenState extends State<ListingScreen> {
    ...
    return GetBuilder<AppState>(
      builder: (appState) {
        final services = appState.searchResults
            ...
            .map<ServiceItem>((s) {
              ...
              return ServiceItem(
                ...
                providerAvatar: s['image']?.toString().isNotEmpty == true
                    ? s['image']
            ...
            .where((service) => _matchesServiceFilter(service))
            .toList();
        ...
        final tasks = appState.searchResults
            ...
            .map<TaskItem>((t) {
              final double priceVal =
                  double.tryParse(t['price']?.toString() ?? '0') ?? 0.0;
              return TaskItem(
                id: t['id']?.toString() ?? '',
                title: t['name'] ?? '',
                ...
                clientAvatar:
                    (t['client'] is Map &&
                        t['client']['avatar_url']?.toString().isNotEmpty ==
                            true)
                    ? t['client']['avatar_url']
                    : 'assets/images/onboard3.jpg',
                clientRating: (t['client'] is Map)
                    ? (double.tryParse(
                            t['client']['rating']?.toString() ?? '',
                          ) ??
                          4.9)
                    : 4.9,
                budget: priceVal,
                ...
                tags: ['On-site'],
                clientReviews: (t['client'] is Map)
                    ? (int.tryParse(
                            t['client']['tasks_count']?.toString() ?? '',
                          ) ??
                          0)
                    : 0,
              );
            })
            .where((task) => _matchesTaskFilter(task, appState))
            .toList();
  ...
  bool _matchesTaskFilter(dynamic task, AppState appState) {
    switch (_activeFilter) {
      ...
      case 'Nearest to me':
        final city = appState.currentUser.location
            .split(',')
            .first
            .trim()
            .toLowerCase();
      ...
      case 'Available Today':
        return task.urgency.toLowerCase() == 'urgent' ||
            task.schedule.toLowerCase().contains('today');
      case 'Under \$100':
  ...
  double? _extractPrice(String priceLabel) {
    final match = RegExp(
      r'(\d+(?:\.\d+)?)',
    ).firstMatch(priceLabel.replaceAll(',', ''));
    if (match == null) {
      return null;
    }
        ...
        children: [
          ...
          SizedBox(
            ...
            child: TextButton.icon(
              ...
              icon: Icon(
                AppStateScope.of(context).isServiceSaved(service.id)
                    ? Icons.bookmark
