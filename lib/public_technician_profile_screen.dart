...
class PublicTechnicianProfileScreen extends StatelessWidget {
  ...
  const PublicTechnicianProfileScreen({Key? key, required this.technicianData})
    : super(key: key);
      ...
      child: Column(
        children: [
          CircleAvatar(
            radius: 50,
            backgroundImage:
                (technicianData['avatar_url'] != null &&
                    technicianData['avatar_url'].toString().isNotEmpty)
                ? NetworkImage(technicianData['avatar_url'])
                : const AssetImage('assets/images/onboard3.jpg')
                      as ImageProvider,
          ),
          ...
          Text(
            technicianData['primary_occupation'] ??
                technicianData['tagline'] ??
        ...
        child: Row(
          children: [
            _buildTrustBadge(
              Icons.fingerprint,
              'Identity Verified',
              technicianData['identity_verified'] == true,
            ),
            const SizedBox(width: 12),
            _buildTrustBadge(
              Icons.workspace_premium,
              'Professional Verified',
              technicianData['professional_verified'] == true,
            ),
            const SizedBox(width: 12),
            _buildTrustBadge(
              Icons.verified_user,
              'Boulot Man Verified',
              technicianData['boulotman_verified'] == true,
            ),
          ],
        ),
      ),
  ...
  Widget _buildStats() {
    return Container(
      ...
      child: Row(
        ...
        children: [
          _buildStatItem(
            '${technicianData['years_experience'] ?? 0} yrs',
            'Experience',
          ),
          _buildStatItem(
            '${technicianData['completed_jobs'] ?? 0}',
            'Jobs Completed',
          ),
  ...
  Widget _buildAboutTab() {
    return ListView(
      ...
      children: [
        ...
        const SizedBox(height: 12),
        if (technicianData['hourly_rate'] != null)
          Text('• Hourly Rate: \$${technicianData['hourly_rate']}'),
        if (technicianData['daily_rate'] != null)
          Text('• Daily Rate: \$${technicianData['daily_rate']}'),
        if (technicianData['starting_price'] != null)
          Text('• Starting Price: \$${technicianData['starting_price']}'),
      ],
    );
  }
}
