...
class _MyTasksScreenState extends State<MyTasksScreen> {
  ...
  void initState() {
    ...
    final hasNoData =
        ...
        (!isClient || appState.clientContracts.isEmpty);
    if (hasNoData) {
      _isLoading = true;
    }
        ...
        return Scaffold(
          ...
          body: Column(
            children: [
              ...
              if (!isClient)
                _buildTabBar(showSaved: false, showContracts: false),
              Expanded(
                child: _activeTab == 'Saved' && isClient
                    ? (savedServices.isEmpty
                          ? _buildSavedEmptyState()
                          : _buildSavedServicesList(savedServices))
                    : _activeTab == 'Contracts' && isClient
                    ? (appState.clientContracts.isEmpty
                          ? _buildEmptyContractsState()
                          : _buildClientContractsList(
                              appState.clientContracts,
                              context,
                              appState,
                            ))
                    : (tasks.isEmpty
                          ? _buildEmptyState(isClient, isCompany)
                          : _buildTasksList(tasks, isClient, isCompany)),
              ),
            ],
          ),
        );
  ...
  Widget _buildTabBar({bool showSaved = true, bool showContracts = false}) {
    ...
    return Container(
      color: Colors.white,
      child: Row(children: tabs.map(_buildTabItem).toList()),
    );
  }
      ...
      child: InkWell(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            border: Border(
              bottom: BorderSide(
                color: isActive ? const Color(0xFFFF4500) : Colors.transparent,
                width: 2,
              ),
            ),
          ),
  ...
  Widget _buildTasksList(
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) =>
          _buildTaskCard(visibleTasks[index], isClient, isCompany),
    );
  }
          ...
          if (isClient && _activeTab != 'Saved')
            ElevatedButton(
              onPressed: () {
                Navigator.of(context).push(
                  MaterialPageRoute(
                    builder: (context) => const PostTaskFormScreen(),
                  ),
                );
              },
              style: ElevatedButton.styleFrom(
          ...
          if (!isClient && _activeTab == 'Active')
            ElevatedButton(
              onPressed: () {
                Navigator.of(context).push(
                  MaterialPageRoute(
                    builder: (context) => const TaskFeedScreen(),
                  ),
                );
              },
              style: ElevatedButton.styleFrom(
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          if (isCompany &&
              (task.status == 'In Progress' || task.status == 'Delivered')) ...[
                    ...
                    child: OutlinedButton.icon(
                      ...
                      onPressed: () {
                        ...
                        final String otherName =
                            task.assignedToName ?? 'Technician';
                        ...
                        if (existingThreadId == null) {
                          appState.createOrOpenThread(
                            ...
                            initialMessage:
                                'Hi $otherName, let\'s chat about task: "${task.title}".',
                          );
                        }
                        Navigator.of(context).push(
                          MaterialPageRoute(
                      ...
                      onPressed: () {
                        showDialog(
                          ...
                          builder: (ctx) => AlertDialog(
                            ...
                            content: Text(
                              'Are you sure you want to mark this task as completed? This will release the escrow funds (\$${task.budget.toStringAsFixed(0)}) to ${task.assignedToName ?? 'the technician'}.',
                            ),
                            actions: [
                              TextButton(
                                  ...
                                  try {
                                    ...
                                    if (context.mounted) {
                                      ...
                                      ScaffoldMessenger.of(
                                        ...
                                        const SnackBar(
                                          content: Text(
                                            'Task completed successfully! Escrow released.',
                                          ),
                                        ),
                                      );
                                    }
  ...
  void _showDeleteConfirmationDialog(BuildContext context, String taskId) {
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: const Text(
          'Are you sure you want to delete this task? This action will cancel any active contracts, release/refund escrow funds, and notify the assigned technician.',
        ),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(ctx),
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 6),
          const Text(
            'Contracts sent by companies will appear here.',
  ...
  Widget _buildClientContractsList(
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        final double budget =
            double.tryParse(contract['budget']?.toString() ?? '0') ?? 0.0;
        final int milestonesCompleted =
            int.tryParse(contract['milestones_completed']?.toString() ?? '0') ??
            0;
        final int milestonesTotal =
            int.tryParse(contract['milestones_total']?.toString() ?? '0') ?? 0;
        final int milestonesReleased =
            int.tryParse(contract['milestones_released']?.toString() ?? '0') ??
            0;
        final double milestoneAmount = milestonesTotal > 0
            ? budget / milestonesTotal
            : budget;
            ...
            child: Column(
              ...
              children: [
                ...
                LinearProgressIndicator(
                  ...
                  backgroundColor: const Color(0xFFF1F5F9),
                  valueColor: AlwaysStoppedAnimation<Color>(
                    progress == 100 ? Colors.green : const Color(0xFFFF4500),
                  ),
                ),
                const SizedBox(height: 16),
                if (paymentStatus == 'in_escrow' &&
                    milestonesCompleted > milestonesReleased)
                  SizedBox(
                    width: double.infinity,
                    child: ElevatedButton(
                      onPressed: () async {
                        await appState.releaseContractEscrow(
                      ...
                      child: Text(
                        'Approve & Release Milestone (\$${milestoneAmount.toStringAsFixed(2)})',
                      ),
                    ),
                  )
                  ...
                  SizedBox(
                    ...
                    child: ElevatedButton(
                      onPressed: () async {
                        await appState.fundContractEscrow(
                          contract['id'].toString(),
                        );
                        ScaffoldMessenger.of(context).showSnackBar(
                          const SnackBar(
                            content: Text('Funds secured in escrow!'),
                          ),
                        );
                      },
                      style: ElevatedButton.styleFrom(
                      ...
                      child: Text(
                        'Fund Escrow (\$${milestoneAmount.toStringAsFixed(2)})',
                      ),
                    ),
                  )
                  ...
                  const Center(
                    child: Text(
                      'Funds in Escrow - Awaiting Completion',
                    ...
                    onPressed: () {
                      ...
                      if (existingThreadId == null) {
                        appState.createOrOpenThread(
                          ...
                          otherPartyImage: '',
                          initialMessage:
                              'Hi $companyName, let\'s chat about contract: "$title".',
                        );
                      }
                      Navigator.of(context).push(
                        MaterialPageRoute(
