          ...
          children: [
            _card('${task['title'] ?? 'Task'}', [
                ...
                children: [
                  ...
                  if (task['created_at'] != null)
                    Text(
                      ...
                      style: const TextStyle(color: taskMuted, fontSize: 12),
                    ),
                ],
              ),
              ...
              Text(
                ...
                style: const TextStyle(color: taskMuted, height: 1.5),
              ),
              ...
              _info(
                'Service type',
                _label(task['service_type'], fallback: 'On-site'),
              ),
            ]),
            ...
            if (lat != null && lng != null)
              _card('Pinned service location', [
                const Text(
                  'This is the exact location selected on the map.',
                  style: TextStyle(color: taskMuted),
                ),
                const SizedBox(height: 10),
                ClipRRect(
  ...
  Widget _attachment(dynamic raw) {
    ...
    if (!image || url.isEmpty)
      return ListTile(
        ...
        title: Text(name),
        subtitle: Text(
          _label(a['file_type'], fallback: 'File'),
          style: const TextStyle(color: taskMuted),
        ),
      );
    return InkWell(
            ...
            ClipRRect(
              ...
              child: Image.network(
                ...
                errorBuilder: (_, __, ___) => const SizedBox(
                  height: 150,
                  child: Center(
                    child: Icon(Icons.broken_image_outlined, color: taskMuted),
                  ),
                ),
              ),
            ),
...
class _ClientTaskEditState extends State<ClientTaskEditScreen> {
  ...
  void initState() {
    ...
    if (t['contact_methods'] is List &&
        (t['contact_methods'] as List).isNotEmpty)
      contacts = (t['contact_methods'] as List).map((x) => '$x').toSet();
    _loadCategories();
  }

  String _skillText(dynamic value) => value is List
      ? value
            .map((x) => x is Map ? '${x['name'] ?? x['title'] ?? ''}' : '$x')
  ...
  Future<void> _pickMapLocation() async {
    final picked = await Navigator.push<SelectedTaskLocation>(
      context,
      MaterialPageRoute(
    ...
    setState(() {
      ...
      if ((picked.address ?? '').isNotEmpty) location.text = picked.address!;
      if ((picked.city ?? '').isNotEmpty) city.text = picked.city!;
    });
  }
    ...
    body: ListView(
      ...
      children: [
        ...
        OutlinedButton.icon(
          ...
          label: Text(
            latitude == null && location.text.trim().isEmpty
        ...
        if (latitude == null || longitude == null)
          _field(location, 'Location / address'),
        ...
        OutlinedButton.icon(
          ...
          label: Text(
            deadline.isEmpty ? 'Choose deadline' : 'Deadline: $deadline',
          ),
          style: OutlinedButton.styleFrom(
            foregroundColor: taskOrange,
            side: const BorderSide(color: taskOrange),
        ...
        SwitchListTile(
          ...
          title: const Text(
            ...
            style: TextStyle(color: taskNavy),
          ),
          value: materials,
          activeThumbColor: taskOrange,
        ...
        const SizedBox(height: 12),
        if (widget.task['attachments'] is List &&
          ...
          ...(widget.task['attachments'] as List).map(
            (a) => ListTile(
              contentPadding: EdgeInsets.zero,
              leading: const Icon(Icons.attach_file, color: taskOrange),
              title: Text('${a['file_name'] ?? 'Attachment'}'),
            ),
          ),
        ],
        OutlinedButton.icon(
