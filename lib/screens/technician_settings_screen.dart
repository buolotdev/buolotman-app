...
class TechnicianSettingsScreen extends StatefulWidget {
  ...
  @override
  State<TechnicianSettingsScreen> createState() => _TechnicianSettingsState();
}
...
class _TechnicianSettingsState extends State<TechnicianSettingsScreen> {
  ...
  Map<String, dynamic> _profile(dynamic value) =>
  ...
  Future<void> _load() async {
    try {
      ...
      final p = u['technician_profile'] is Map
          ? Map<String, dynamic>.from(u['technician_profile'])
          : u;
  ...
  Future<void> _changePassword() async {
    if (currentPassword.text.isEmpty ||
      ...
      return _notice(
        'Enter your current password, then a matching new password of at least 8 characters.',
      );
  ...
  Future<void> _delete() async {
    ...
    try {
      ...
      _notice('We could not delete your account.');
    }
  }
    ...
    body: loading
        ...
        : ListView(
            ...
            children: [
              ...
              _card('Privacy and notifications', [
                _unsupported(
                  'Visible in technician search',
                  visibleInSearch,
                  (v) => setState(() => visibleInSearch = v),
                ),
                _unsupported(
                  'Show phone to clients',
                  showPhone,
                  (v) => setState(() => showPhone = v),
                ),
                _unsupported(
                  'Show email to clients',
                  showEmail,
                  (v) => setState(() => showEmail = v),
                ),
                _unsupported(
                  'Accept urgent jobs',
                  acceptUrgent,
                  (v) => setState(() => acceptUrgent = v),
                ),
                _unsupported(
                  'Email notifications',
                  emailNotifications,
                ...
                const Padding(
                  ...
                  child: Text(
                    'These controls are displayed by the website, but the current backend does not expose persistence fields for them.',
    ...
    child: Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          ...children.map(
            (w) => Padding(padding: const EdgeInsets.only(top: 8), child: w),
          ),
        ],
      ),
    ),
  );
  Widget _unsupported(String label, bool value, ValueChanged<bool> onChanged) =>
      SwitchListTile(
        contentPadding: EdgeInsets.zero,
        title: Text(label),
        subtitle: const Text(
          'Unavailable until the backend adds this preference.',
