...
class _BidsManagementState extends State<TechnicianBidsManagementScreen> {
  ...
  List<Map> _filtered() {
    ...
    result.sort((a, b) {
      ...
      return '${b['submitted_at'] ?? b['created_at'] ?? ''}'.compareTo(
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(const SnackBar(content: Text('Bid withdrawn.')));
        setState(() => future = api.myBids());
      }
    } catch (_) {
      if (mounted)
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('We could not withdraw this bid.')),
        );
    }
  }
  ...
  Future<void> _messageClient(Map bid) async {
    ...
    if (client == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Client messaging is unavailable for this bid.'),
        ),
      );
      return;
    }
    try {
      ...
      if (mounted)
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('We could not open a conversation with the client.'),
          ),
        );
    }
  }
      ...
      body: FutureBuilder<dynamic>(
        ...
        builder: (_, snapshot) {
          ...
          if (snapshot.hasError)
            return const Center(child: Text('Unable to load your bids.'));
          data = snapshot.data;
          final all = _filtered();
  ...
  Widget _card(Map bid) {
    ...
    final skills = bid['skills'] is List
        ? (bid['skills'] as List).join(', ')
        : '${bid['skills'] ?? ''}';
    return Card(
      elevation: 0,
          ...
          children: [
            Row(
              children: [
                Expanded(
                  child: Text(
                    '${bid['task_title'] ?? bid['title'] ?? 'Task #$taskId'}',
                    style: const TextStyle(
                      color: Color(0xFF001F3F),
                      fontSize: 17,
                      fontWeight: FontWeight.w800,
            ...
            const SizedBox(height: 8),
            Text(
              '${bid['location'] ?? bid['city'] ?? 'Location not specified'} • ${bid['competing_bids'] ?? bid['competingBids'] ?? 0} competing bids',
              style: const TextStyle(color: Color(0xFF64748B)),
            ),
            const SizedBox(height: 10),
            Text(
              'Your proposal: ${bid['message'] ?? bid['proposal'] ?? 'No message'}',
            ...
            if ('${bid['extra_notes'] ?? bid['extra'] ?? ''}'.isNotEmpty)
              Text(
                'Extra notes: ${bid['extra_notes'] ?? bid['extra']}',
                style: const TextStyle(color: Color(0xFF64748B)),
              ),
                ...
                OutlinedButton.icon(
                  onPressed: taskId == null
                      ...
                      : () => Navigator.push(
                          ...
                          MaterialPageRoute(
                            builder: (_) =>
                                TechnicianTaskDetailScreen(taskId: taskId),
                          ),
                        ),
