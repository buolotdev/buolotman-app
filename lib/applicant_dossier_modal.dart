...
class _ApplicantDossierModalState extends State<ApplicantDossierModal> {
  ...
  void _messageApplicant() async {
    ...
    final TextEditingController msgController = TextEditingController(
      text:
          "Hello, your profile is incomplete. Please provide your missing details (e.g., ID document, Address) so we can verify your account.",
    );
      ...
      try {
        ...
        if (mounted) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text(
                "Message sent! Check your Messages tab to continue the chat.",
              ),
            ),
          );
        }
  ...
  void _verifyUser() async {
    ...
    try {
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text("Account approved and verified successfully."),
          ),
        );
      }
    } catch (e) {
  ...
  void _rejectAccount() async {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(SnackBar(content: Text("Suspension error: $e")));
      }
    } finally {
      if (mounted) {
        setState(() => _isProcessing = false);
  ...
  Widget build(BuildContext context) {
    ...
    final String country =
        user['location']?.toString().split(',').last.trim() ?? 'Global';
    ...
    final Color statusColor = isVerified
        ...
        : const Color(0xFFEab308); // Orange/Yellow
