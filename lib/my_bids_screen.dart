...
class _MyBidsScreenState extends State<MyBidsScreen> {
    ...
    return GetBuilder<AppState>(
      builder: (appState) {
        ...
        final roleBids = appState.bids.where((bid) {
          return bid.role == appState.currentRole ||
              bid.bidderName == appState.currentUser.name;
        }).toList();
        ...
        final visibleBids = roleBids.where((bid) {
          ...
          final matchesSearch =
              query.isEmpty ||
              taskText.contains(query) ||
              bid.skill.toLowerCase().contains(query);

          final isAccepted = bid.isAccepted;
          final isTaskDeleted =
              task != null &&
              (task.status == 'Deleted' || task.status == 'Cancelled');
          final isArchived =
              isAccepted || isTaskDeleted || bid.role != appState.currentRole;
                  ...
                  child: SingleChildScrollView(
                    ...
                    child: Column(
                      children: [
                        ...
                        visibleBids.isEmpty
                            ? _buildEmptyState()
                            : _buildBidsList(appState, visibleBids),
                      ],
                    ),
                  ),
                ),
  ...
  Widget _buildTabBar() {
    return Container(
      ...
      child: Row(
        children: [_buildTabItem('Active'), _buildTabItem('Archived')],
      ),
    );
  }
  ...
  Widget _buildStats(AppState appState, List<dynamic> visibleBids) {
    ...
    final total = visibleBids.fold<double>(
      0,
      (sum, bid) => sum + (bid.price as double),
    );
                      ...
                      if (bid.rating > 0)
                        Row(
                          children: [
                            ...
                            const SizedBox(width: 4),
                            Text(
                              '${bid.rating.toStringAsFixed(1)} (${bid.reviews})',
                              style: const TextStyle(
                                fontSize: 12,
                                color: Color(0xFF64748B),
                              ),
        ...
        Expanded(
          child: ElevatedButton(
            onPressed: () {
              Navigator.of(context).push(
                MaterialPageRoute(
                  builder: (context) => SubmitBidScreen(taskId: task.id),
                ),
              );
            },
            style: ElevatedButton.styleFrom(
