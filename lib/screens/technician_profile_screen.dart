...
class _TechnicianProfileScreenState extends State<TechnicianProfileScreen> {
  ...
  final slots = const [
    (
      'front',
      'National ID — front',
      'Front side of your national ID or passport.',
      'id',
    ...
    (
      'back',
      'National ID — back',
      'Back side of your national ID or passport.',
      'id',
      Icons.badge_outlined,
    ),
    (
      ...
      'Trade license, diploma, or professional certificate.',
      'certificate',
    ...
    (
      ...
      'Clear recent selfie. Backend stores this as an ID document.',
      'id',
  ...
  dynamic _doc(String key) {
    final words = key == 'front'
        ? ['front']
        : key == 'back'
        ? ['back']
        : key == 'certificate'
        ? ['certificate', 'license', 'trade']
        : ['selfie', 'photo', 'portrait'];
    for (final d in documents) {
      final title = '${d['title'] ?? ''}'.toLowerCase();
      if (words.any(title.contains)) return d;
    }
    return null;
  }
  ...
  Future<void> _pick(String key) async {
    ...
    if (file.bytes == null) {
      _notice('Could not read that file. Please choose it again.');
      return;
    }
  ...
  Future<void> _submit() async {
    if (selected.isEmpty) {
      _notice('Attach at least one document first.');
      return;
    }
    ...
    try {
      for (final slot in slots) {
        ...
        final old = _doc(slot.$1);
        if (old != null && old['id'] != null)
          await api.deleteTechnicianDocument(old['id']);
        final upload = await api.uploadTechnicianDocumentBytes(
          bytes: file.bytes!,
          filename: file.name,
        );
        final url = upload['file_url'];
        if (url is! String || url.isEmpty)
          throw const ApiException('Upload returned no file URL.', 500);
      ...
      if (mounted) {
        setState(() {});
        _notice('Documents submitted for admin review.');
      }
    } catch (e) {
      if (mounted)
        _notice(
          e is ApiException
              ? e.message
              : 'We could not submit the documents. Please try again.',
        );
    } finally {
  ...
  Future<void> _delete(dynamic doc) async {
    ...
    if (ok != true) return;
    try {
      await api.deleteTechnicianDocument(doc['id']);
      documents = await api.technicianDocuments();
      if (mounted) setState(() {});
    } catch (e) {
      if (mounted)
        _notice(
          e is ApiException ? e.message : 'We could not delete this document.',
        );
    }
  }
    ...
    return FutureBuilder<Map<String, dynamic>>(
      ...
      builder: (_, snap) {
        ...
        final verified =
            ...
            data['technician_profile']?['is_verified'] == true;
        return Card(
          elevation: 0,
          child: Padding(
            padding: const EdgeInsets.all(18),
            child: Row(
              children: [
                Icon(
                  ...
                  color: verified ? Colors.green : const Color(0xFFD97706),
                  size: 32,
                ),
        ...
        child: Column(
          ...
          children: [
            ...
            Row(
              children: [
                TextButton.icon(
                  onPressed: submitting ? null : () => _pick(slot.$1),
                  icon: Icon(
                    doc == null ? Icons.attach_file : Icons.refresh,
                    color: orange,
                  ),
                  label: Text(
                    doc == null ? 'Attach' : 'Replace',
                    style: const TextStyle(color: orange),
                  ),
                ),
                if (doc != null)
                  TextButton.icon(
                    onPressed: submitting ? null : () => _delete(doc),
  ...
  Widget _local(PlatformFile f) => InkWell(
    onTap: () => _preview(
      f.name,
      _image(f.name)
          ? Image.memory(f.bytes!, fit: BoxFit.contain)
          : const Icon(Icons.picture_as_pdf, color: Colors.white, size: 90),
    ),
    child: Row(
      children: [
        _image(f.name)
            ? Image.memory(f.bytes!, width: 74, height: 58, fit: BoxFit.cover)
            : const Icon(Icons.picture_as_pdf, color: orange, size: 42),
        const SizedBox(width: 10),
        Expanded(
          child: Text(
            f.name,
        ...
        const Icon(Icons.open_in_full, color: muted),
      ],
    ),
  );
  Widget _remote(dynamic d) {
    final path = '${d['file_url'] ?? ''}';
    final url = api.resolveImageUrl(path);
    return InkWell(
      onTap: () => _preview(
        '${d['title']}',
        _image(path)
  ...
  void _preview(String title, Widget child) => showDialog(
    context: context,
    builder: (_) => Dialog(
