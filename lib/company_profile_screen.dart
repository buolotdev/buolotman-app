...
class CompanyProfileScreen extends StatelessWidget {
                      ...
                      children: [
                        ...
                        if (!_isPublicView) ...[
                          if (appState.companyRegistrationStatus != null) ...[
                            ...
                            _buildStatusCard(
                              title:
                                  appState.companyProfile?['registration_status']
                                          ?.toString() ==
                                      'verified'
                                  ? 'Verified'
                                  : 'Pending Review',
                              subtitle:
                                  appState.companyProfile?['registration_number']
                                          ?.toString()
                                          .isNotEmpty ==
                                      true
  ...
  Widget _buildOverview(AppState appState, Map<String, dynamic> profile) {
    ...
    final List<dynamic> capabilities = profile['capabilities'] is List
  ...
  Widget _buildServiceCatalog(
    ...
    if (_isPublicView) {
      ...
      final services = offered is List
  ...
  Widget _buildTeamSection(AppState appState, Map<String, dynamic> profile) {
    final int teamSize =
  ...
  Widget _buildRatings(
    ...
    final int reviewCount =
        int.tryParse(profile['review_count']?.toString() ?? '0') ??
        reviews.length;
  ...
  Widget _buildPrimaryActions(
    ...
    if (_isPublicView) {
      ...
      Future<void> requestQuote() async {
        ...
        final ok = await showDialog<bool>(
          ...
          builder: (d) => AlertDialog(
            ...
            actions: [
              ...
              ElevatedButton(
                onPressed: () =>
                    Navigator.pop(d, service.text.trim().isNotEmpty),
        ...
        if (ok == true) {
          try {
            await ApiService().submitCompanyQuote(companyData!['id'], {
              'service': service.text.trim(),
              'budget': budget.text.trim(),
              'deadline': deadline.text.trim(),
              'project_summary': summary.text.trim(),
            });
            if (context.mounted)
              ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(
                  content: Text('Quote request sent to the company.'),
                ),
              );
          } catch (e) {
            if (context.mounted)
